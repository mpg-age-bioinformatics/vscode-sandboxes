...
function run(argv) {
  ...
  const versionLabel = argv[1];
  const defaultVersion = argv[2];
  const defaultProject = argv[3];
  ...
  alert.messageText = "Create an " + appName + " project";
  alert.informativeText = "Review the settings, then choose Setup.";
  alert.addButtonWithTitle("Setup");
  alert.addButtonWithTitle("Cancel");

  const view = $.NSView.alloc.initWithFrame($.NSMakeRect(0, 0, 620, 112));
  const version = textField(defaultVersion, $.NSMakeRect(145, 80, 465, 24));
  ...
  view.addSubview(label(versionLabel, $.NSMakeRect(0, 84, 135, 20)));
  ...
  writeValue(outputDirectory, "version", ObjC.unwrap(version.stringValue));
